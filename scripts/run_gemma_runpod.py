#!/usr/bin/env python3
"""Run the Gemma judge evaluation with the Qwen/Llama notebook AMP flow."""
...
def load_and_prepare_traces(args) -> list:
    ...
    traces = []
    for path in sorted(preprocessed_dir.glob("*.json")):
    ...
    print(f"Built {len(traces)} AMP Trace objects")
    return traces
...
def run_generation(model, tokenizer, prompt: str, args) -> str:
    rendered_prompt = render_prompt_text(tokenizer, prompt)
    tokenized = tokenizer(text=rendered_prompt, return_tensors="pt", truncation=True)
    model_inputs = {
...
def init_output_artifacts(results_path: Path, debug_log_path: Path, reset_outputs: bool = True):
    if reset_outputs:
...
def run_trial(traces, args):
    results_path = args.output_dir / "gemma_3_12b_results.csv"
    debug_log_path = args.output_dir / "gemma_3_12b_failures.jsonl"
    evaluators = build_evaluators(args)
    jobs = build_jobs(traces, evaluators)
    init_output_artifacts(results_path, debug_log_path, reset_outputs=not args.resume)

    print(f"Local judge model: {args.model}")
    print("Parameter size: 12B")
    ...
    progress = tqdm(total=len(jobs), desc="Gemma 3 12B run", unit="job")
    ...
    try:
        for job in jobs:
            ...
            for attempt in range(1):
                ...
                try:
                    raw_output = run_generation(model, tokenizer, prompt, args)
                    last_raw_output = raw_output
                    normalized_output = extract_json_object(raw_output)
                    last_normalized_output = normalized_output
                    total_latency_ms += (time.perf_counter() - t0) * 1000.0
                    parsed_result, last_error = job.evaluator._parse_and_validate(normalized_output)
                ...
                if parsed_result is not None:
                    break
                append_failure_log(debug_log_path, {
                    "trace_id": job.trace_id,
                    "evaluator": job.evaluator_name,
                    "level": job.level,
                    ...
                    "normalized_output": last_normalized_output,
                })
            ...
            append_result_row(results_path, result_to_row(job, parsed_result, total_latency_ms, args.model))
...
def parse_args():
    parser = argparse.ArgumentParser(description="Run Gemma 3 12B AMP evaluation on Runpod.")
    parser.add_argument("--model", default=LOCAL_JUDGE_MODEL)
    parser.add_argument("--dataset", default="PatronusAI/TRAIL")
    parser.add_argument("--dataset-split", default="gaia")
    parser.add_argument("--output-dir", type=Path, default=Path("/workspace/gemma_run"))
    ...
    parser.add_argument("--resume", action="store_true", help="Keep existing output files instead of recreating them.")
...
def main():
    args = parse_args()
    traces = load_and_prepare_traces(args)
    summary = run_trial(traces, args)
